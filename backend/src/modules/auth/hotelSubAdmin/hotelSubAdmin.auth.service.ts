/**
 * FILE: src/modules/auth/hotelSubAdmin/hotelSubAdmin.auth.service.ts
 * PURPOSE: Business logic for hotel sub-admin authentication
 *
 * WHAT IT DOES:
 * - Authenticates hotel sub-admin by email/password
 * - Generates JWT token on successful login
 * - Validates tokens and adds them to blacklist on logout
 * - Checks hotel sub-admin account status (is_active, is_blocked, deleted_at)
 *
 * USAGE:
 ...
 * const logout = await logoutHotelSubAdmin(token, adminId);
 *
 * DATABASE TABLES:
 * - hotel_sub_admins: Hotel sub-admin accounts (email, password, is_active, is_blocked, deleted_at, role_id, hotel_id)
...
/**
 * Authenticates hotel sub-admin by email and password
 *
 * WORKFLOW:
 * 1. Find hotel sub-admin in database by email
 ...
 * 6. Generate JWT token with payload {id: hotel_sub_admin_id, role: 'HOTEL_SUB_ADMIN'}
 * 7. Return token and admin object with role_id and hotel_id
 *
 * @param {string} email - Hotel sub-admin email address
 * @param {string} password - Plain text password
 * @returns {{token: string, admin: {hotel_sub_admin_id, email, role, is_active, role_id, hotel_id}}} Login result
 * @throws {Error} EMAIL_NOT_FOUND, INVALID_PASSWORD, ACCOUNT_INACTIVE, ACCOUNT_DELETED
 *
 * @example
 * try {
 *   const result = await loginHotelSubAdmin("subadmin@grandhotel.com", "password123");
 *   console.log("Token:", result.token);
 *   console.log("Admin ID:", result.admin.hotel_sub_admin_id);
 *   console.log("Role ID:", result.admin.role_id);
 * } catch (error) {
 ...
 * }
 */
export async function loginHotelSubAdmin(email: string, password: string) {
  // Find hotel sub-admin by email
  ...
  return {
    token,
    admin: {
      hotel_sub_admin_id: admin.hotel_sub_admin_id,
      email: admin.email,
      name: admin.name,
      role: "HOTEL_SUB_ADMIN",
 ...
 * }
 */
export async function logoutHotelSubAdmin(token: string, adminId: number) {
  // Verify token
