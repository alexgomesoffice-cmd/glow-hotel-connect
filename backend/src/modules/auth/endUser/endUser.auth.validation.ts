/**
 * FILE: src/modules/auth/endUser/endUser.auth.validation.ts
 * PURPOSE: Input validation for end user authentication
 *
 * WHAT IT DOES:
 ...
 *
 * USAGE:
 * import { validateLoginInput, validateLogoutInput } from './endUser.auth.validation';
 * const result = validateLoginInput(email, password);
 * if (!result.isValid) console.log(result.errors);
 */

/**
 * Validates email and password input for end user login
 *
 * CHECKS:
...
/**
 * Validates Authorization header format for end user logout
 *
 * CHECKS:
