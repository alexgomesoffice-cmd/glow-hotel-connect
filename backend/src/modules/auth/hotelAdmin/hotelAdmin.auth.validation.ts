/**
 * FILE: src/modules/auth/hotelAdmin/hotelAdmin.auth.validation.ts
 * PURPOSE: Input validation for hotel admin authentication
 *
 * WHAT IT DOES:
 ...
 *
 * USAGE:
 * import { validateLoginInput, validateLogoutInput } from './hotelAdmin.auth.validation';
 * const result = validateLoginInput(email, password);
 * if (!result.isValid) console.log(result.errors);
 */

/**
 * Validates email and password input for hotel admin login
 *
 * CHECKS:
 ...
 *
 * @example
 * const result = validateLoginInput("admin@grandhotel.com", "password123");
 * if (!result.isValid) {
...
/**
 * Validates Authorization header format for hotel admin logout
 *
 * CHECKS:
