/**
 * FILE: src/modules/auth/hotelAdmin/hotelAdmin.auth.controller.ts
 * PURPOSE: HTTP request/response handlers for hotel admin authentication
 *
 * WHAT IT DOES:
...
import {
  validateLoginInput,
  validateLogoutInput,
} from "./hotelAdmin.auth.validation";
import { extractToken } from "@/utils/token";

/**
 * Handles POST /api/auth/hotel-admin/login request
 *
 * REQUEST BODY:
 * {
 ...
 * 4. Call loginHotelAdmin(email, password)
...
/**
 * Handles POST /api/auth/hotel-admin/logout request
 *
 ...
 * 6. Call logoutHotelAdmin(token, adminId)
...
export async function logoutController(
  ...
  try {
    ...
    // Call service
    const result = await logoutHotelAdmin(token, adminId);
