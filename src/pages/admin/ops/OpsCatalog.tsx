// Generic catalog page (Cities, Hotel Types, Amenities, Bed Types).
import { useMemo, useState } from "react";
import { Plus, Search } from "lucide-react";
import { OpsSectionHeader, OpsTable, OpsTd, OpsTh } from "@/components/admin/ops/primitives";
import { Button } from "@/components/ui/button";

interface Row { id: number; name: string; slug: string; count: number; }

const DATASETS: Record<string, { title: string; description: string; rows: Row[] }> = {
  cities: {
    title: "Cities",
...
const OpsCatalog = ({ kind }: { kind: keyof typeof DATASETS }) => {
  const data = DATASETS[kind];
  const [q, setQ] = useState("");
  const rows = useMemo(() => data.rows.filter((r) => r.name.toLowerCase().includes(q.toLowerCase())), [data, q]);
    ...
    <div className="mx-auto max-w-[1200px] space-y-4 px-6 py-5">
      <OpsSectionHeader
        ...
        right={
          <Button size="sm" className="h-8 gap-1.5 text-xs">
            <Plus className="h-3.5 w-3.5" /> New {data.title.replace(/s$/, "")}
          </Button>
        }
      />
      ...
      <OpsTable>
        <thead>
          <tr>
            <OpsTh>Name</OpsTh>
            <OpsTh className="w-40">Slug</OpsTh>
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="hover:bg-secondary/40">
              ...
              <OpsTd className="font-mono text-xs text-muted-foreground">{r.slug}</OpsTd>
              <OpsTd className="text-right font-mono text-xs tabular-nums">{r.count}</OpsTd>
              <OpsTd className="text-right"><button className="text-xs text-primary hover:underline">Edit</button></OpsTd>
            </tr>
          ))}
        </tbody>
      </OpsTable>
